...
class ProtoData(metaclass=_AutoRegisterProto):
    ...
    @abstractmethod
    def pack_req(self):
        """ Protocol objects packaged to send data as a stream of bytes
    ...
    def unpack_req(self, buf, offset=0):
        """ Unpacking from Byte Streams

        :param buf：byte stream data
        :param offset：Byte Stream Data Offset
        :return：True Unpacked successfully；False unpacking failure
        """
        return True

    # @abstractmethod
    def pack_resp(self):
        """ Protocol Object Packaging
    ...
    def unpack_resp(self, buf, offset=0):
        """ Unpacking from byte streams into return values and associated properties
...
seq_id:{5:d}, is_ack:{6:d}, need_ack:{7:d}>".format(self._sender, self._receiver, self._cmdset, self._cmdid,
    ...
    def pack(self, is_ack=False):
        """ Msg Message Packaging

        :param is_ack: bool: Is it an ack message
        :return: bytearray，message byte stream
        """
        self._len = 13
        try:
            if self._proto:
            ...
            logger.debug("Msg: pack, cmset:0x{0:02x}, cmdid:0x{1:02x}, proto: {2}, "
    ...
    def unpack_protocol(self):
        """ Decode the protocol and its contents from its own buf data.
        """
            ...
            try:
                if self._is_ack:
                    if not self._proto.unpack_resp(self._buf):
                        logger.debug("Msg: unpack_protocol, msg:{0}".format(self))
                        return False
                else:
                ...
                logger.debug("Msg: unpack_protocol, {0} failed e {1}".format(self._proto.__class__.__name__, e))
                raise
        else:
            logger.debug("Msg: unpack_protocol, cmdset:0x{0:02x}, cmdid:0x{1:02x}, class is not registerin registered_\
                           protos".format(self._cmdset, self._cmdid))
            pass
        logger.debug("Msg: unpack_protocol, not registered_protocol, cmdset:0x{0:02x}, cmdid:0x{1:02x}".format(
...
class TextMsg(MsgBase):
    ...
    def unpack_protocol(self):
        ...
        if not self._proto.unpack_resp(self._buf):
            logger.debug("TextMsg: unpack_protocol, msg:{0}".format(self))
            return False
        return True
...
def decode_msg(buff, protocol="v1"):
    if protocol == "v1":
        if len(buff) < 4:
            logger.debug("decode_msg, recv buf is not enouph.")
            return False, buff

        if buff[0] != 0x55:
            logger.debug("decode_msg, magic number is invalid.")
            return False, buff

        if algo.crc8_calc(buff[0:3]) != buff[3]:
            logger.debug("decode_msg, crc header check failed.")
            return False, buff

        msg_len = (buff[2] & 0x3) * 256 + buff[1]
        if len(buff) < msg_len:
            logger.debug("decode_msg, msg data is not enough, msg_len:{0}, buf_len:{1}".format(msg_len, len(buff)))
            return False, buff
        ...
        # filter out '\0xcc'
        if buff[0] == 204:
            logger.debug("decode_msg: recv invalid data, buff {0}".format(buff))
...
class ProtoGetProductVersion(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            self._version = None
            logger.debug("ProtoGetProductVersion, unpack_resp, retcode {0}".format(self._retcode))
            return False
...
class ProtoFcSubInfoReq(ProtoData):
    ...
    def pack_req(self):
        buf = bytearray(self._req_size + 1)#len(self.data_num * 4))
        buf[0] = self._bagid
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoFcSubInfoReq: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoChassisStickOverlay(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoChassisStickOverlay: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoGimbalCtrlSpeed(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoGimbalCtrlSpeed: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoPlaySound(ProtoData):
    ...
    def __init__(self):
        ...
        self._task_ctrl = 0
        self._sound_id = 0
        self._play_ctrl = 1  # 0: Stop playback 1: Interrupt playback 2: Fusion playback 3: Ignore playback
        self._interval = 0
        self._play_times = 0
    ...
    def unpack_resp(self, buf, offset=0):
        self._retcode = buf[offset]
        logger.debug("ProtoPlaySound unpack_resp, buf : {0}, self._retcode: {1}".format(binascii.hexlify(buf), self._retcode))
        if self._retcode == 0:
...
class ProtoVisionDetectStatus(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoVisionDetectType: unpack_resp, error")
            return False
...
class ProtoVisionSetColor(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoVisionSetColor: unpack_resp, retcode {0}".format(self._retcode))
            return False
...
class ProtoPositionMove(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoPositionMove: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoSetWheelSpeed(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoSetWheelSpeed: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoVisionDetectEnable(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoVisionDetectEnable: unpack_resp, error:{0}".format(self._error))
            return False
...
class ProtoVisionDetectInfo(ProtoData):
    ...
    def unpack_req(self, buf, offset=0):
        ...
        if self._type == 0:  # shoulder
            ...
            logger.debug("unsupported type: {0}".format(self._type))
...
class ProtoSubscribeAddNode(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0 or self._retcode == 0x50:
            ...
            logger.debug("ProtoSubscribeAddNode: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoAddSubMsg(ProtoData):
    ...
    def pack_req(self):
        ...
        for i in range(0, self._sub_data_num):
            logger.debug("ProtoSubMsg: UID:{0}".format(hex(self._sub_uid_list[i])))
            struct.pack_into("<Q", buf, 5 + 8 * i, self._sub_uid_list[i])
        struct.pack_into("<H", buf, 5 + 8 * self._sub_data_num, self._sub_freq)
        logger.debug("ProtoSubMsg: pack_req, num:{0}, buf {1}".format(self._sub_data_num, binascii.hexlify(buf)))
        return buf
...
class ProtoServoCtrlSet(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoServoCtrlSet: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class ProtoRoboticArmMoveCtrl(ProtoData):
    ...
    def unpack_resp(self, buf, offset=0):
        ...
        if self._retcode == 0:
            ...
            logger.debug("ProtoRoboticArmMoveCtrl: unpack_resp, retcode:{0}".format(self._retcode))
            return False
...
class TextProtoData(object):
    ...
    def pack_req(self):
        ...
        :return: Byte stream data.
        """
        logger.debug("TextProtoData: pack_req text_cmd {0}, type {1}".format(self.text_cmd, type(self.text_cmd)))
        self._buf = self.text_cmd
        return self._buf

    def unpack_req(self, buf, offset=0):
        """ Unpacking from a byte stream.

        :param buf：Byte stream data.
        :param offset：Byte stream data offset.
        :return：True Unpacked successfully；False Unpacking failed.
        """
    ...
    def unpack_resp(self, buf, offset=0):
        """ Unpacking from a byte stream into a return value and associated attributes.
    ...
    @property
    def proresp(self):
        """ Preprocesses responses to the aceleration?, attitude?, and temp? commands.
        ...
        if len(resp.split("~")) == 2:
            ...
            logger.debug("doesn't support sdk! proresp returns empty dict")
        return msg_dict
